...
// Driver's ambulance — first entry from the list (driver has 1 assigned ambulance)
const ambulanceListParams = ref({ limit: 1 })
...
} = useAssignedAmbulanceServiceList(
  acceptedParams,
  computed(() => !!driverAmbulanceId.value),
)
...
const isAnyError = computed(
  () =>
    ...
    ambulanceListQuery.isError.value ||
    acceptedQuery.isError.value ||
    inServiceQuery.isError.value,
)
