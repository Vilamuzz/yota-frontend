...
<template>
  <DashboardLayout>
    <template #title>Pemasukan Program Sosial</template>
    ...
    <div class="space-y-6">
      ...
      >
        ...
        <template #headers>
          ...
          <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Nama Program</th>
          <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Total Pemasukan</th>
          <th class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider">Pelanggan Aktif</th>
        ...
        <template #rows>
          ...
          >
            ...
            <td class="px-6 py-4 whitespace-nowrap font-medium text-right text-gray-600 dark:text-gray-200">
              {{ formatCurrency(program.collectedFund ?? 0) }}
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-center text-gray-600 dark:text-gray-200">
              {{ program.totalSubscribers }}
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-center">
              <span :class="['inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border', getStatusColor(program.status)]">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center">
              <BaseIconButton
                :to="{ name: 'dashboard-social-programs-income-detail', params: { id: program.id } }"
