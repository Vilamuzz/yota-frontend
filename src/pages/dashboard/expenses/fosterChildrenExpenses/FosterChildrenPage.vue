...
const dummyExpenses = ref<FosterChildrenExpense[]>([
  {
    id: '1',
...
const expensesSource = computed(() => {
  return useDummy.value
    ? dummyExpenses.value
    : fosterChildrenExpenses.value
})

const expenseData = computed(() => {
  const map = new Map()
  expensesSource.value.forEach((trx: FosterChildrenExpense) => {
...
const { fosterChildrenExpenseListQuery, fosterChildrenExpenses, pagination } = useFosterChildrenExpenseList(id, queryParams)

const displayChildren = computed(() => expenseData.value)
...
<template>
  <DashboardLayout>
    ...
    <BaseTable
      class="mt-6"
      :loading="fosterChildrenExpenseListQuery.isPending.value && displayChildren.length === 0"
    ...
    >
      ...
      <template #headers>
        ...
        <th class="px-6 py-3 text-center text-xs font-poppins uppercase tracking-wider">Total Pengeluaran</th>
        <th class="px-6 py-3 text-center text-xs font-poppins uppercase tracking-wider">
          Periode
        </th>
