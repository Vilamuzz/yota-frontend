...
// ── Computed errors (local zod + server) ──────────────────────────────────
...
function handleImageChange(event: Event) {
  ...
  if (!allowed.includes(file.type)) {
    fieldErrors.value.coverImage = 'Hanya JPG, PNG, atau WebP yang diizinkan.'
    return
  }
  if (file.size > 5 * 1024 * 1024) {
    fieldErrors.value.coverImage = 'Ukuran gambar maksimal 5 MB.'
    return
  }
...
function handleSubmit(overrideStatus?: MediaStatus) {
  ...
  const imageErrors: Record<string, string> = form.coverImageFile
    ? {}
    : { coverImage: 'Gambar sampul wajib diunggah.' }
...
<template>
  <DashboardLayout>
    <template #title>Tambah Berita</template>
      ...
      <form @submit.prevent="handleSubmit()" class="space-y-6">
        ...
        >
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-gray-200 dark:divide-gray-700">
            <!-- Left column: Cover image + Title + Category + Status -->
