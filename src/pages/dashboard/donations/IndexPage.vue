...
import { Eye, SquarePen, Trash2, HandHeart, Plus } from 'lucide-vue-next'
...
const { donationListQuery } = useDonationList(queryParams)
...
const handleCreate = () => {
  router.push({ name: 'dashboard-donations-create' })
}

const handleView = (donation: Donation) => {
  // TODO: navigate to donation detail page or open modal
  console.log('View donation:', donation.id)
}

const handleEdit = (donation: Donation) => {
  router.push({ name: 'dashboard-donations-edit', params: { id: donation.id } })
}
</script>
