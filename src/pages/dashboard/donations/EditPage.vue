...
watch(
  ...
  (data) => {
    if (!data) return
    const donation = (data as { data?: Donation })?.data ?? (data as unknown as Donation)
    title.value = donation.title ?? ''
    description.value = donation.description ?? ''
    category.value = donation.category ?? ''
    fundTarget.value = donation.fund_target ? String(donation.fund_target) : ''
    // Normalise date to YYYY-MM-DD for the date input
    dateEnd.value = donation.date_end ? (donation.date_end.split('T')[0] ?? '') : ''
    // Show existing image as preview (URL, not a File)
    if (donation.image_url) imagePreview.value = donation.image_url
  },
  { immediate: true },
)
