...
export const usePrayerList = (
  slug: MaybeRefOrGetter<string>,
  queryParams?: MaybeRefOrGetter<PrayerQueryParams>,
) => {
  const listQuery = useQuery<PrayerListResponse, ApiError>({
    queryKey: ['prayers', slug, queryParams],
    queryFn: () =>
      prayerService.getListPrayer(toValue(slug), queryParams ? toValue(queryParams) : undefined),
