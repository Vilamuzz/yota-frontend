...
export const useAmbulanceHistoryCreate = () => {
  ...
  const createMutation = useMutation<
    ...
    CreateAmbulanceHistoryRequest
  >({
    mutationFn: (data: CreateAmbulanceHistoryRequest) =>
      ambulanceHistoryService.createAmbulanceHistory(data),
    onSuccess: () => {
  ...
  >({
    mutationFn: (data: CreateAmbulanceHistoryRequest) =>
      ambulanceHistoryService.createAmbulanceHistoryDriver(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['adminAmbulanceHistories'] })
      queryClient.invalidateQueries({ queryKey: ['driverAmbulanceHistories'] })
      queryClient.invalidateQueries({ queryKey: ['driverHistorySummary'] })
  ...
  const validationErrors = computed(
    () =>
      createMutation.error.value?.response?.data?.validation ??
      createDriverMutation.error.value?.response?.data?.validation ??
      null,
  )
